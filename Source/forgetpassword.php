<?php
	...
	if($email_check != '') {
		$sql = 'SELECT * FROM account WHERE Email = "'.$email_check.'"';
		$result = executeResult($sql);
		if($result != null && count($result) > 0) {
			header('Location: verify.php');
		} else {
			$note = '<b style="color: red">Email không tồn tại trên hệ thống! Vui lòng thử lại</b>';
		}
	}
?>
<!DOCTYPE html>
			...
			<div class="col-md-8 col-xs-12">
				...
				<form method="post">
					<div class="form-group">
						<label for="email_check">Email đăng ký tài khoản:<span style="color: red;">*</span></label>
						<input id="email_check" type="email" name="email_check" class="form-control" required placeholder="Nhập Email đăng ký tài khoản">
					</div>
					<button type="submit" class="btn btn-success">Gửi</button>
					<div style="margin-top: 20px">Chúng tôi sẽ gửi mã xác nhận về email của bạn .Vui lòng nhập chính xác email.(<span style="color: red;">Có thể mất một vài phút để nhận được mã xác nhận</span>)</div>
				</form>
			</div>
		</div>
	</div>
